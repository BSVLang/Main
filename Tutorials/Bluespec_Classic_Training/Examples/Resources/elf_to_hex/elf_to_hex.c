...
{
    uint64_t bytes_per_raw_mem_word  = bits_per_raw_mem_word / 8;    // 4
    ...
    fprintf (fp, "@%06" PRIx64 "    // raw_mem addr;  byte addr: %08" PRIx64 "\n",
	...
	fprintf (fp, "@%06" PRIx64 "    // last raw_mem addr;  byte addr: %08" PRIx64 "\n",
		 ((addr - BASE_ADDR_B) / bytes_per_raw_mem_word),
		 addr - BASE_ADDR_B);
	for (int j = (bytes_per_raw_mem_word - 1); j >= 0; j--)
	    fprintf (fp, "%02x", 0);
...
void print_usage (FILE *fp, int argc, char *argv [])
{
    ...
    fprintf (fp, "    0x%08" PRIx64 " ..  0x%08" PRIx64,
	     MIN_MEM_ADDR_256MB, MAX_MEM_ADDR_256MB);
    fprintf (fp, " (size 0x%08" PRIx64 " = %0" PRId64 " bytes)\n",
	     MAX_MEM_ADDR_256MB - MIN_MEM_ADDR_256MB,
	     MAX_MEM_ADDR_256MB - MIN_MEM_ADDR_256MB);
}
